...
from schemas import TodoBase, TodoIn, TodoOut, TodoOutUser, TodoUpdate, ResultIn
...
def all_todo(skip: int = 0, limit: int = 10,  db: Session = Depends(get_db), current_user: Session = Depends(logged_in)):
    all = todo_service.get_with_pagination(db=db, skip=skip, limit=limit, descending=True, count_results=True)
...
def user_todo(skip: int = 0, limit: int = 10,  db: Session = Depends(get_db), current_user: Session = Depends(logged_in)):
    ...
    all = todo_service.get_by_key(db=db, skip=skip, limit=limit, descending=True, count_results=True, user_id=user_id)
    return handle_result(all)
